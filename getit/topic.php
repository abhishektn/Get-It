...
function showUser(str) {
  ...
  xmlhttp.open("GET","reply.php?id=' . $row['topic_id'] . '"+str,true);
  xmlhttp.send();
}
</script>';
					...
					echo '</br><div class="panel panel-default">
							...
							<div class="panel-body">
								...
								<p class="list-group-item "><span class="glyphicon glyphicon-hand-right"></span>      ' . htmlentities(stripslashes($posts_row['post_content'])) . '</p>
								 <a class="btn btn-default" href="#reply"><span class="glyphicon  glyphicon-edit"></span> Answer</a>
								</div>
			...
			else
			{
				...
				echo '<form method="post" action="reply.php?id=' . $row['topic_id'] . '" id="reply">
							<div class="form-group">
							  <label for="comment"><h3>Answer here:</h3></label>
							  <textarea class="form-control" rows="6" id="comment" name="reply-content" placeholder="Enter your Answer" required></textarea>
							</div>
						 <input type="submit" class="btn btn-default" onchange="showUser(this.value) value="Submit Answer"/>
					</form>';
			} 
